...
public class PlayerAttributes : MonoBehaviour
{
  ...
  void Update()
  {
    ...
    if (!deathFaceOn && !isLive) { OpenDeathScreen(); }
  ...
  {
    if (other.gameObject.tag == "Drop" && isLive)
    {
      InventoryHotBar.addItem(other.gameObject.GetComponent<ItemDrop>().item, other.gameObject.GetComponent<ItemDrop>().quant);
  ...
  void respawn()
  {
    ...
    isLive = true;
    CloseDeathScreen();
    InventoryHotBar.clearAndDropAll(gameObject);
